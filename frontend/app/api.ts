...
export interface Character { id: number; name: string; role: string; age_description: string; appearance: string; clothing: string; personality: string; description: string; visual_prompt: string; reference_image_url: string | null; reference_generation_attempt: number; consistency_prompt: string; }
...
export interface CreditBalance { balance: number; monthly_allowance: number; used: number; }
export interface UsageSummary { projects: number; scenes: number; character_references: number; assemblies: number; credits_consumed: number; }
...
export async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!["GET", "HEAD", "OPTIONS"].includes(method) && path !== "/auth/csrf/") headers.set("X-CSRFToken", await ensureCsrfToken());
  ...
  if (!response.ok) {
    const message = typeof data === "object" && data !== null && "detail" in data ? String((data as { detail: unknown }).detail) : response.status === 401 || response.status === 403 ? "Your session has expired. Please sign in again." : "Request failed.";
