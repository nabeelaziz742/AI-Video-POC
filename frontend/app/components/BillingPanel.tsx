...
export default function BillingPanel() {
  ...
  async function load() {
    const [planData, subData, creditData] = await Promise.all([
      api<{ plans: Plan[] }>("/billing/plans/"),
      api<Subscription>("/billing/subscription/"),
      api<CreditBalance>("/credits/"),
    ]);
    setPlans(planData.plans); setSubscription(subData); setCredits(creditData);
  }

  useEffect(() => { load().catch(() => setMessage("Unable to load billing information.")); }, []);
