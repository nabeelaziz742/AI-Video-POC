...
interface Props { project: VideoProject; onSelect: (project: VideoProject) => void; }
...
export function VersionHistory({ project, onSelect }: Props) {
  ...
  async function createVersion() {
    ...
    try {
      const next = await api<VideoProject>(`/projects/${project.id}/versions/`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt: prompt.trim(), title: project.title, input_type: project.input_type, duration: project.duration, aspect_ratio: project.aspect_ratio }) });
      setVersions((current) => [...current, next].sort((a, b) => a.version_number - b.version_number));
      onSelect(next);
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to create a new version."); }
    finally { setBusy(false); }
  }

  return <section className="mt-5 rounded-2xl border border-white/10 bg-white/[0.02] p-4">
    <div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">Version history</h3><p className="mt-1 text-xs text-white/30">Editing creates a new version. Older videos stay intact.</p></div><span className="rounded-full bg-white/5 px-2.5 py-1 text-[10px] uppercase tracking-wider text-white/40">V{project.version_number}</span></div>
    <div className="mb-4 flex flex-wrap gap-2">{versions.map((version) => <button key={version.id} type="button" onClick={() => onSelect(version)} className={`rounded-lg border px-3 py-2 text-xs ${version.id === project.id ? "border-violet-400/40 bg-violet-400/10 text-white" : "border-white/10 text-white/45 hover:text-white"}`}>Version {version.version_number}{version.status === "completed" ? " · Ready" : version.status === "failed" ? " · Failed" : ""}</button>)}</div>
