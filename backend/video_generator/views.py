...
class VideoProjectCreateView(APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            project = VideoProject.objects.create(title=title, prompt=prompt, input_type=input_type, aspect_ratio=aspect_ratio, duration=duration, status=VideoProject.Status.QUEUED, provider="fal_pixverse_c1")
