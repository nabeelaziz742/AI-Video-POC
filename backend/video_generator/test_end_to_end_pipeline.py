...
class EndToEndPipelineTests(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="pipeline-user", password="StrongPass123")
        ...
        self.scene = VideoScene.objects.create(project=self.project, scene_number=1, duration=10, prompt="Hero walks home", status=VideoScene.Status.PLANNED)
        self.scene.characters.add(self.character)

    def post(self, path):
        request = self.factory.post(path, {}, format="json"); request.user = self.user; return request

    def get(self, path):
        request = self.factory.get(path); request.user = self.user; return request
    ...
    def test_assembly_then_final_status_completes_project(self, create_movie):
        ...
        with patch("video_generator.views.JSON2VideoService.get_movie") as get_movie:
            ...
            response = VideoProjectStatusView.as_view()(self.get("/status/"), project_id=self.project.id)
