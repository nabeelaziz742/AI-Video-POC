...
class SceneGenerateView(APIView):
    def post(self, request, project_id, scene_id):
        ...
        references = [
            ...
            for index, character in enumerate(scene.characters.filter(reference_image_url__isnull=False).order_by("id"), start=1)
        ]
        if not references:
        ...
        try:
            ...
            scene.status = VideoScene.Status.FAILED; scene.error_message = str(exc); scene.save(update_fields=["status", "error_message"])
            return Response({"detail": str(exc)}, status=status.HTTP_502_BAD_GATEWAY)
        scene.status = VideoScene.Status.PROCESSING; scene.provider = provider_name; scene.provider_project_id = job["request_id"]; scene.video_url = None; scene.error_message = None
...
class SceneStatusView(APIView):
    def get(self, request, project_id, scene_id):
        ...
        try:
            result = get_video_provider(scene.provider).get_scene_result(scene.provider_project_id)
        except VideoProviderError as exc:
            scene.status = VideoScene.Status.FAILED; scene.error_message = str(exc); scene.save(update_fields=["status", "error_message"])
            return Response({"detail": str(exc)}, status=status.HTTP_502_BAD_GATEWAY)
        if result["status"] == "completed":
            ...
            if not video_url:
                scene.status = VideoScene.Status.FAILED; scene.error_message = "Provider marked the scene completed but returned no video URL."
            else:
                scene.status = VideoScene.Status.COMPLETED; scene.video_url = video_url; scene.error_message = None
            scene.save(update_fields=["status", "video_url", "error_message"])
        elif result["status"] in {"queued", "processing"}:
            scene.status = VideoScene.Status.PROCESSING; scene.save(update_fields=["status"])
        elif result["status"] in {"failed", "error", "cancelled"}:
            scene.status = VideoScene.Status.FAILED; scene.error_message = result.get("error") or "AI video generation failed."; scene.save(update_fields=["status", "error_message"])
        return Response(VideoSceneSerializer(scene).data)
...
class SceneRegenerateView(SceneGenerateView):
    def post(self, request, project_id, scene_id):
        ...
        scene.status = VideoScene.Status.PLANNED; scene.provider_project_id = None; scene.video_url = None; scene.error_message = None
...
class ProjectAssembleView(APIView):
    def post(self, request, project_id):
        ...
        try:
            ...
            project.status = VideoProject.Status.FAILED; project.error_message = "Video assembly provider failed."; project.save(update_fields=["status", "error_message", "updated_at"])
            return Response({"detail": str(exc)}, status=status.HTTP_502_BAD_GATEWAY)
        project.provider = "json2video"; project.provider_project_id = result["project"]; project.status = VideoProject.Status.PROCESSING; project.error_message = None
