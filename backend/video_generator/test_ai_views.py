...
class SceneGenerationSafetyTests(TestCase):
    ...
    def test_generation_uses_provider_without_calling_real_api(self, mock_provider):
        response = SceneGenerateView.as_view()(self.request("/generate/"), project_id=self.project.id, scene_id=self.scene.id)
        self.assertEqual(response.status_code, 202); self.scene.refresh_from_db(); self.assertEqual(self.scene.status, VideoScene.Status.PROCESSING); self.assertEqual(self.scene.provider_project_id, "fake-job"); mock_provider.assert_called_once_with("fal_pixverse_c1")
