...
class JSON2VideoServiceTests(SimpleTestCase):
    ...
    def test_create_movie_from_clips_sorts_scenes_and_builds_payload(self, post):
        response = Mock()
        ...
        self.assertEqual(payload["scenes"][0]["comment"], "Scene #1")
        self.assertEqual(payload["scenes"][0]["elements"][0]["src"], "https://example.com/1.mp4")
        self.assertEqual(payload["scenes"][1]["comment"], "Scene #2")
        self.assertEqual(payload["client-data"]["project_id"], 7)

    @patch.dict("os.environ", {"JSON2VIDEO_API_KEY": "test-key"})
    def test_missing_scene_video_url_is_rejected(self):
        service = JSON2VideoService()
        with self.assertRaises(RuntimeError):
            service.create_movie_from_clips(
                clips=[{"scene_number": 1, "video_url": None}],
                width=1080,
                height=1920,
                project_id=7,
            )
